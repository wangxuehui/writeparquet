...
public class BasketParquetWriter {
	...
	private void generateBasketData(String outFilePath) throws IOException {
		...
		config.clear();
		config.addResource(new Path("/etc/hadoop/conf.cloudera.yarn/core-site.xml"));
		config.addResource(new Path("/etc/hadoop/conf.cloudera.yarn/hdfs-site.xml"));
		DataWritableWriteSupport.setSchema(schema, config);
		Path outDirPath = new Path("hdfs://192.168.0.80/user/hive/warehouse/parquet_example1/year=2015/month=7/day=16/"+outFilePath);
